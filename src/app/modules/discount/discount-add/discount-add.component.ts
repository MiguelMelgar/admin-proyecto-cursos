...
export class DiscountAddComponent implements OnInit {
  ...
  isLoading: any;
  today: string = new Date().toISOString().split('T')[0]; // Define la fecha de hoy en formato 'YYYY-MM-DD'
  ...
  // Método para manejar el cambio en la fecha de inicio
  onStartDateChange(): void {
    // Reinicia la fecha de fin si la fecha de inicio cambia
    if (this.start_date) {
      this.end_date = null;
    }
  }
  ...
  removeCategorie(i: number): void {
    this.categorie_selecteds.splice(i, 1);
  }

  selectedTypeDiscount(value: any): void {
    this.type_discount = value;
  }

  selectedTypeCampaing(value: any): void {
    this.selectedTypeCoupon(1);
    this.type_campaing = value;
  }

  selectedTypeCoupon(value: any): void {
